...
class GitHttpInterfaceNonexportedTest < ApplicationSystemTestCase
  def setup
  ...
  def test_admins_can_read_and_write
    ...
    assert(
      ...
      "Admins should be able to clone private repos"
    )

    assert(
      push_a_change_on("one_commit"),
      "Admins should be able to push to private repos"
    )
  end
  ...
  def test_permitted_user_can_read_and_write
    ...
    assert(
      ...
      "Permitted users should be able to clone private repos"
    )

    assert(
      push_a_change_on("one_commit"),
      "Permitted users should be able to push to private repos"
    )
  end
  ...
  def test_unpermitted_user_cannot_read_or_write
    ...
    refute(
      git_clone(git_url("one_commit.git", "user", "p")),
      "Unpermitted users shouldn't be able to clone private repos"
    )
  end
  ...
  def test_guest_cannot_read_or_write
    ...
    refute(
      ...
      "Guests shouldn't be able to clone private repos"
    )
  end
